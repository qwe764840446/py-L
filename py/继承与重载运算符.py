...
class Mylist:  #定义类Mylist
    ...
    def __add__(self,x):   #定义操作+
        for i in range(len(self.__mylist)):  #依次遍历列表，执行加操作
            self.__mylist=self.mylist+x

        return self.__mylist

    def __sub__(self,x):
        for i in range(0, len(self.__mylist)):  # 依次遍历列表，执行减操作
            self.__mylist[i] = self.__mylist[i] - x
        return self.__mylist

    def __mul__(self,x):
        for i in range(0, len(self.__mylist)):  # 依次遍历列表，执行乘操作
            self.__mylist[i] = self.__mylist[i] * x
        return self.__mylist
    def __div__(self,x):
        for i in range(0, len(self.__mylist)):  # 依次遍历列表，执行除操作
            self.__mylist[i] = self.__mylist[i] / x
        return self.__mylist
    def __mod__(self,x):
        for i in range(0, len(self.__mylist)):  # 依次遍历列表，执行求余操作
            self.__mylist[i] = self.__mylist[i] % x
        return self.__mylist
    def __pow__(self,x):
        for i in range(0, len(self.__mylist)):  # 依次遍历列表，计算x次方
            self.__mylist[i] = self.__mylist[i] ** x
        return self.__mylist
...
print(list1.__add__(10))
print(list1.__sub__(10))
print(list1.__mul__(10))
print(list1.__mod__(10))
print(list1.__pow__(10))
print(list1.show())
